...
test('falls back to the user endpoint when the org endpoint 404s', async () => {
  ...
  expect(result.isOk()).toBe(true);
  if (result.isOk()) expect(result.value[0]).toMatchObject({ owner: 'blimmer', name: 'solo' });
});
...
test('getRepoLanguages returns bytes keyed by language', async () => {
  const client = new FakeGithubClient();
  client.onRequest('GET /repos/{owner}/{repo}/languages', {}).resolves({ TypeScript: 1000, JavaScript: 200 });
