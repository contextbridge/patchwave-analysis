...
import { type BundleKind, type Uploader, formatUploadError } from '../upload/Uploader.ts';

const SUPPORT_LINE = 'Reach us at founders@contextbridge.com — or learn more at https://patchwave.ai';

export type ShareChoice = 'markdown' | 'full' | 'declined';
...
export type ShareOutcome =
  | { kind: 'shared'; choice: 'markdown' | 'full'; uploadId: string; identifier: string }
  | { kind: 'declined' }
  | { kind: 'cancelled' }
  | { kind: 'upload-failed'; choice: 'markdown' | 'full'; message: string };

export async function runSharePrompt(inputs: SharePromptInputs): Promise<ShareOutcome> {
  const { prompter, analytics } = inputs;

  prompter.note(
    [
      `Markdown report:  ${inputs.markdownPath}`,
  ...
  const choiceResult = await prompter.select<ShareChoice>({
    ...
    choices: [
      ...
      { value: 'markdown', label: 'Share the markdown summary only', hint: 'the .md, no raw data' },
      { value: 'full', label: 'Share the markdown + raw data', hint: 'the full .zip you saw above' },
    ],
  });
  ...
  const kind: BundleKind = choice === 'markdown' ? 'markdown' : 'zip';
  const bytes = choice === 'markdown' ? new TextEncoder().encode(inputs.markdownContent) : inputs.zipBytes;

  const spinner = prompter.spinner();
  spinner.start('Uploading...');
  const uploadResult = await inputs.uploader.upload({
    bytes,
    kind,
    identifier,
    appVersion: inputs.appVersion,
    timestamp: inputs.now.toString(),
  });
  ...
  if (uploadResult.isErr()) {
    ...
    prompter.note(
      [`Your local files are unchanged:`, `  ${inputs.markdownPath}`, `  ${inputs.zipPath}`, '', SUPPORT_LINE].join(
        '\n',
      ),
      "We couldn't upload",
    );
