...
const MD_BYTES = new TextEncoder().encode('# report\n');
...
describe('UploaderImpl', () => {
  ...
  test("markdown kind: posts kind:'markdown' and PUTs raw bytes with text/markdown", async () => {
    const { fetch, calls } = recordFetch([presignResponse(), new Response('', { status: 200 })]);

    const result = await new UploaderImpl({ endpoint: ENDPOINT, fetch }).upload(makeInput('markdown', MD_BYTES));

    expect(result.isOk()).toBe(true);
    const postBody = JSON.parse(calls[0]?.init?.body as string) as Record<string, unknown>;
    expect(postBody).toMatchObject({ kind: 'markdown', sizeBytes: MD_BYTES.byteLength });
    expect(calls[1]?.init?.body).toBe(MD_BYTES);
    expect((calls[1]?.init?.headers as Record<string, string>)['content-type']).toBe('text/markdown');
  });
