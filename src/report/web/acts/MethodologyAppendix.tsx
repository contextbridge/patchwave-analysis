  ...
  return (
    ...
    >
      <details open={open} onToggle={(e) => setOpen(e.currentTarget.open)}>
        <summary
          data-testid={methodologyAppendixTestIds.summary}
          className="text-foreground hover:text-muted-foreground cursor-pointer text-base font-semibold tracking-tight select-none"
        >
          ...
          >
            {appendixTabs.map((tab) => (
              <button
                ...
                data-testid={`${methodologyAppendixTestIds.tab}-${tab.id}`}
      ...
      <div className="border-border text-muted-foreground mt-12 border-t pt-5 text-xs">
        ...
        <a
          ...
          data-testid={methodologyAppendixTestIds.repoLink}
        ...
        <a
          ...
          data-testid={methodologyAppendixTestIds.contextbridgeLink}
          className="text-primary underline underline-offset-4"
        >
          ContextBridge
        </a>
      ...
      {rows.length === 0 ? (
        ...
        <ul className="space-y-1">
          {rows.map((row) => (
            <li
              key={row.login}
              data-testid={`${methodologyAppendixTestIds.rawDataPerson}-${countLabel}-${row.login}`}
